...
We welcomed our second cohort of 31 MEDS students to NCEAS just one week ago! They spent much of their first few days not only reviewing a whole lot of math, but also started learning how to navigate the RStudio IDE, were introduced to using git/GitHub for version control, began a bit of function writing, data exploration, and data visualization, and created their first reproducible reports using RMarkdown (so a pretty jam-packed week to start off our year!).

As we neared the end of this first week, Dr. Allison Horst introduced the class to [Quarto](www.quarto.org), a scientific publishing tool that feels and looks a whole lot like RMarkdown -- something that allows users to combine prose, code, and outputs into a single rendered (and importantly, reproducible) document. These can be standalone documents, or users can combine multiple documents to form books, websites, slides and more.
